#!/bin/bash
# 3x-ui 安装脚本（简体中文提示版）
# 来源：https://raw.githubusercontent.com/mhsanaei/3x-ui/master/install.sh
# 仅翻译交互提示和状态信息，安装逻辑、参数、路径及下载地址保持不变。
...
# stable within this localized script.
prompt_or_default() {
...
install_postgres_local() {
    ...
    case "${release}" in
        ...
        alpine)
            ...
            ;;
        *)
            echo -e "${red}不支持为此发行版自动安装 PostgreSQL：${release}${plain}" >&2
            return 1
            ;;
    esac
...
setup_ssl_certificate() {
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${yellow}请确认端口 80 已开放，稍后使用 x-ui 命令重试${plain}"
...
setup_ip_certificate() {
    ...
    echo -e "${yellow}默认监听端口为 80。如选择其他端口，请确保外部端口 80 转发到该端口。${plain}"
    ...
    if [[ "${WebPort}" -ne 80 ]]; then
        echo -e "${yellow}提醒：Let's Encrypt 仍会连接端口 80；请将外部端口 80 转发到 ${WebPort}。${plain}"
    fi
    ...
    while true; do
        if is_port_in_use "${WebPort}"; then
            ...
            local alt_port=""
            if [[ "$NONINTERACTIVE" == "1" ]]; then
                echo -e "${red}端口 ${WebPort} 忙碌，无法在非交互模式下继续。${plain}"
                return 1
            fi
            read -rp "请输入 acme.sh 独立监听器的其他端口（留空则中止）：" alt_port
            alt_port="${alt_port// /}"
            if [[ -z "${alt_port}" ]]; then
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${yellow}请确认端口 ${WebPort} 可访问（或已从外部端口 80 转发）${plain}"
...
ssl_cert_issue() {
    ...
    if [[ "$NONINTERACTIVE" == "1" ]]; then
        ...
        if [[ -z "$domain" ]] || ! is_domain "$domain"; then
            echo -e "${red}XUI_SSL_MODE=domain 要求有效的 XUI_DOMAIN（当前值：'${XUI_DOMAIN:-}'）。${plain}"
            return 1
        fi
    else
        while true; do
    ...
    if ~/.acme.sh/acme.sh --list 2> /dev/null | awk '{print $1}' | grep -Fxq "${domain}"; then
        ...
        if [[ -n "${acmeCertDir}" ]]; then
            ...
            echo -e "${yellow}发现 ${domain} 的现有证书，将重复使用。${plain}"
    ...
    local WebPort=80
    prompt_or_default WebPort "请选择使用的端口（默认为 80）：" "80" XUI_ACME_HTTP_PORT
    if [[ -z ${WebPort} ]]; then
        WebPort=80
...
prompt_and_setup_ssl() {
    ...
    echo -e "${green}1.${plain} 域名 Let's Encrypt（有效期 90 天，自动续期）"
    echo -e "${green}2.${plain} IP 地址 Let's Encrypt（有效期 6 天，自动续期）"
    echo -e "${green}3.${plain} 自定义 SSL 证书（现有文件路径）"
    echo -e "${green}4.${plain} 跳过 SSL（高级选项，仅限反向代理 / SSH 隧道）"
    echo -e "${blue}注意：${plain} 选项 1 和 2 要求端口 80 开放，选项 3 需要手动填写路径。"
    echo -e "${blue}注意：${plain} 选项 4 通过普通 HTTP 提供面板，仅在 nginx/Caddy 或 SSH 隧道后方使用才安全。"
    if [[ "$NONINTERACTIVE" == "1" ]]; then
        ...
        read -rp "请选择（IP 默认选项 2）：" ssl_choice
    ...
    case "$ssl_choice" in
        1)
            ...
            if ssl_cert_issue; then
                ...
                if [[ -n "${cert_domain}" ]]; then
                    ...
                    echo -e "${green}✓ 已成功配置域名 SSL 证书：${cert_domain}${plain}"
                else
        ...
        2)
            ...
            if [[ "$NONINTERACTIVE" != "1" ]]; then
                local ip_confirm=""
                read -rp "${server_ip} 是此服务器接收流量的公网 IPv4 地址吗？【默认 y】：" ip_confirm
        ...
        3)
            ...
            echo -e "${yellow}注意：这些证书文件需要您在外部自行续期。${plain}"

            systemctl restart x-ui > /dev/null 2>&1 || rc-service x-ui restart > /dev/null 2>&1
            ;;
        4)
            echo ""
            echo -e "${red}⚠ 面板将以不带 SSL/TLS 的方式安装。${plain}"
            echo -e "${yellow}登录凭据和 Cookie 将通过明文 HTTP 传输。${plain}"
            echo -e "${yellow}仅在以下情况下安全：${plain}"
            echo -e "${yellow}  • 由反向代理（nginx、Caddy、Traefik）代您终止 TLS，或${plain}"
            ...
            if [[ "$NONINTERACTIVE" == "1" ]]; then
                ...
                read -rp "是否仅将面板绑定到 127.0.0.1？（推荐，可强制使用 SSH 隧道 / 反向代理访问）【y/N】：" bind_local
            fi
            if [[ "$bind_local" == "y" || "$bind_local" == "Y" ]]; then
                ${xui_folder}/x-ui setting -listenIP "127.0.0.1" > /dev/null 2>&1
                SSL_HOST="127.0.0.1"
                echo -e "${green}✓ 面板已仅绑定到 127.0.0.1，公网现在无法访问。${plain}"
                echo ""
                echo -e "${green}SSH 端口转发：在本地计算机执行以下命令访问面板：${plain}"
                echo -e "  标准 SSH 命令："
                ...
                echo ""
                echo -e "${yellow}另一种方式：将反向代理（nginx/Caddy）指向 127.0.0.1:${panel_port}，由它终止 TLS。${plain}"
            else
                echo -e "${yellow}面板将通过普通 HTTP 监听所有网络接口，请确保前方的其他组件负责终止 TLS。${plain}"
            fi
...
config_after_install() {
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            echo -e "  2) PostgreSQL（推荐客户端数量多或节点多时使用）"
            if [[ "$NONINTERACTIVE" == "1" ]]; then
            ...
            if [[ "$db_choice" == "2" ]]; then
                ...
                while [[ -z "$xui_dsn" ]]; do
                    ...
                    if [[ "$pg_mode" == "2" ]]; then
                        ...
                        echo -e "${yellow}正在安装 PostgreSQL，此过程可能需要一些时间……${plain}"
                        local pg_cred_file
                        pg_cred_file=$(mktemp 2> /dev/null) || pg_cred_file=$(mktemp -t x-ui-pg-creds.XXXXXXXX)
...
EOF
            ...
            if [[ "$db_choice" == "2" ]]; then
                ...
                echo -e "${yellow}  “备份”会下载 pg_dump 生成的 .dump 文件；“恢复”通过 pg_restore 重新加载该文件。${plain}"
            fi
            ...
            if [[ "$db_choice" == "2" && "$pg_local_installed" == "1" ]]; then
                ...
                echo -e "${yellow}⚠ 请保存密码，其他位置不会再以明文存储。${plain}"
...
setup_fail2ban() {
    ...
    if [[ ! -x /usr/bin/x-ui ]]; then
        echo -e "${yellow}未找到 x-ui 命令行工具，跳过 Fail2ban 自动设置。${plain}"
        return 0
    fi
    ...
    if /usr/bin/x-ui setup-fail2ban; then
        ...
        echo -e "${yellow}Fail2ban 设置未完成；在运行 “x-ui” 并打开 IP 限制菜单前，IP 限制功能将保持禁用。继续执行。${plain}"
    fi
    return 0
}
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ ! -s ${xui_folder}-linux-$(arch).tar.gz ]]; then
            ...
            echo -e "${red}下载的 x-ui 发布压缩包为空${plain}"
            exit 1
        fi
    else
        tag_version=$1
        ...
        if [[ "$tag_version" == "dev" || "$tag_version" == "dev-latest" ]]; then
            ...
            echo -e "${yellow}正在安装滚动开发版本（标签：dev-latest）。这是每次提交生成的预发布版本，不是稳定版。${plain}"
        else
    ...
    curl -fLRo "${xui_script_temp}" https://raw.githubusercontent.com/Arm0ne/CN3XUI/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
    ...
    if [[ -e ${xui_folder}/ ]]; then
        ...
        if [[ -d "${xui_folder}/bin" ]]; then
            ...
            if ! mv "${xui_folder}/bin" "${custom_bin_backup}"; then
                custom_bin_backup=""
                echo -e "${yellow}无法备份 bin/，本次更新不会保留其中的自定义文件${plain}"
            fi
        fi
    ...
    if [[ $? -ne 0 ]]; then
        ...
        rm -f "${xui_script_temp}"
        echo -e "${red}解压 x-ui 发布压缩包失败。旧安装已被移除，修复前面板无法启动；请重新运行安装程序${plain}"
        exit 1
    fi
    rm x-ui-linux-$(arch).tar.gz -f
    ...
    if [[ $? -ne 0 || ! -s x-ui ]]; then
        rm -f "${xui_script_temp}"
        echo -e "${red}解压后的 x-ui 压缩包缺少 x-ui 二进制文件。旧安装已被移除，修复前面板无法启动；请重新运行安装程序${plain}"
        exit 1
    fi
    chmod +x x-ui
    chmod +x x-ui.sh
    ...
    if [[ -n "${custom_bin_backup}" ]]; then
        ...
        if [[ ${#restored_custom_bin[@]} -gt 0 ]]; then
            echo -e "${green}已恢复本版本未提供的 bin/ 自定义文件：${restored_custom_bin[*]}${plain}"
        fi
    fi
    trap - EXIT INT TERM
...
│  ${blue}x-ui enable${plain}       - 启用系统启动时自动启动          │
│  ${blue}x-ui disable${plain}      - 禁用系统启动时自动启动          │
