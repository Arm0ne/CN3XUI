#!/bin/bash
# 3x-ui 管理脚本（简体中文菜单版）
# 来源：https://raw.githubusercontent.com/mhsanaei/3x-ui/main/x-ui.sh
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/Arm0ne/CN3XUI/main/install-cn.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    if [[ $? == 0 ]]; then
        # The upstream updater installs its English management script. Restore
        # the localized script immediately after the panel update succeeds.
        replace_xui_script "https://raw.githubusercontent.com/Arm0ne/CN3XUI/main/x-ui.sh" "false" || LOGE "中文管理菜单恢复失败，请稍后选择菜单 4 重试。"
...
update_menu() {
    ...
    if replace_xui_script "https://raw.githubusercontent.com/Arm0ne/CN3XUI/main/x-ui.sh" "false"; then
